...
stp_map_funct_nat_reg <- function(indicator, indicator_desc) {
  ...
  sub_data <-
    stp %>%
    inner_join(STP_need_demand %>% select(-stp20nm), by = c("stp20cd"), keep = FALSE) %>%
...
# CCG
ccg_map_funct_nat <- function(indicator, indicator_desc) {

  sub_data <-
    ccg %>%
    inner_join(CCG_need_demand %>% select(-ccg20nm), by = c("ccg20cd"), keep = FALSE)  %>%
    select(1:9, midlands_flag, indicator, geometry) %>%
    rename(value = indicator)
...
ccg_map_funct_nat("emot_dis_ratio", "Emotional disorders in CYP - Need:demand ratio")
ccg_map_funct_nat("cond_ratio", "Conduct disorders in CYP - Need:demand ratio")
ccg_map_funct_nat("hyperkin_ratio", "Hyperkinetic disorders in CYP - Need:demand ratio")
ccg_map_funct_nat("self_harm_ratio", "Self harm in CYP - Need:demand ratio")

...
#'
#' @export
cypmh_model_run_mlp <- function(cypmh) {
  u <- levels(cypmh$util_description)

  x_train <- cypmh %>%
    dplyr::select(-.data$util_description) %>%
    as.matrix()
  y_train <- keras::to_categorical(as.numeric(cypmh$util_description) - 1, length(u))
  colnames(y_train) <- u
  ...
  m <- keras::keras_model_sequential() %>%
    ...
    keras::layer_dense(units = length(u), activation = "softmax")
